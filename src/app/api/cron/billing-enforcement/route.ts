 ...
 *
 * Rules:
 *  1. 7-day warning — users with paymentFailedAt between 6-8 days ago (window to
 *     avoid duplicate sends if cron runs slightly off schedule)
 ...
 *     - Push '__offline__' to KV so the Worker shows an offline page
 *     - Delete custom domain KV entries so custom domains 404
 *     - Send deactivation email
 *  3. Unpaid/canceled cleanup — users with status 'unpaid' or 'canceled'
 *     with no deactivatedAt yet (fallback for Stripe-side cancellations)
 */
...
export async function GET(request: NextRequest) {
  ...
  const stats = { warned: 0, deactivated: 0, errors: 0 }
  ...
  console.log(`[billing-enforcement] Done — warned: ${stats.warned}, deactivated: ${stats.deactivated}, errors: ${stats.errors}`)
