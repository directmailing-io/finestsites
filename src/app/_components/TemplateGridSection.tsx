...
function TemplateCard({ tpl, idx }: { tpl: TemplateCardData; idx: number }) {
  ...
  const label = tpl.isAllrounder ? 'Allgemein' : (tpl.nmCompanies[0] ?? '')
  ...
  return (
    ...
    >
      {/* Preview */}
      <div style={{ height: 260, background: pastel, position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
        {cover ? (
      ...
      <div style={{ padding: '16px 20px 20px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        {label && (
          <p style={{ fontSize: 11, fontWeight: 700, color: '#aaa', letterSpacing: '0.06em', textTransform: 'uppercase', margin: '0 0 6px' }}>
            {label}
          </p>
        )}
        <h3 style={{ fontSize: 16, fontWeight: 700, color: '#111', lineHeight: 1.3, margin: 0, flex: 1 }}>
          {tpl.title}
        </h3>
        <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 12, color: '#bbb' }}>{tpl.domain}</span>
...
export default function TemplateGridSection({ templates }: { templates: TemplateCardData[] }) {
  ...
  // Only show available (non-coming-soon) templates
  const available = useMemo(() => templates.filter(t => !t.isComingSoon), [templates])

  // Filter chips only when there are multiple companies
  ...
  const filtered = useMemo(() => {
    ...
    if (activeFilter === 'Allgemein') return available.filter(t => t.isAllrounder)
