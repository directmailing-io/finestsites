...
function AppSidebar({ activeIdx = 0 }: { activeIdx?: number }) {
  ...
  return (
    <div style={{ width: 46, background: '#1a2530', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 0 8px', gap: 2, flexShrink: 0 }}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src="/logos/logo-white.png" alt="FinestSites" style={{ width: 32, height: 11, objectFit: 'contain', display: 'block', opacity: 0.85, marginBottom: 10 }} />
      {icons.map((icon, i) => (
        <div key={i} style={{ width: 34, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 6, background: i === activeIdx ? 'rgba(212,197,226,0.2)' : 'transparent', color: i === activeIdx ? '#D4C5E2' : 'rgba(255,255,255,0.35)' }}>
          {icon}
        </div>
      ))}
    </div>
